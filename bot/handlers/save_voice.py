...
async def handle_voice(message: types.Message, state: FSMContext):
    ...
    await message.reply("🎙️ Voice received! Now, send me a text description or keywords for this voice.", reply=True)
...
async def handle_description(message: types.Message, state: FSMContext, db_user: BotUser):
    ...
    try:
        ...
        await message.reply("✅ Voice saved successfully! You can now search for it using inline mode.")
    except IntegrityError:
        # If the user already saved this exact voice, we could just update the description, but let's keep it simple.
        await message.reply("⚠️ You have already saved this voice message.")
    except Exception as e:
        await message.reply("❌ Failed to save voice. Please try again.")
    finally:
        await state.clear()

@router.message(SaveVoiceStates.waiting_for_description, ~F.text)
async def handle_invalid_description(message: types.Message):
    await message.reply("Please send text only for the description.")
